...
**2026-08-28 - local search, agent, and accessibility pass.** The canonical search identity is now `Sevan Lewis-Payne | Data & Software Engineer`, with a current description, a cross-browser multi-format favicon family, WebSite/ProfilePage/Person structured data, expanded raw HTML, `llms.txt`, and a truthful public `agents.md`. The React experience retains an H1 and a visible keyboard bypass around WebGL, while desktop windows and Python status surfaces expose stronger semantics. The monitor glass no longer uses the four-corner vignette. The supplied agent scan still identifies an external WAF/crawler-access gate; Cloudflare and production were not changed. Evidence is recorded in the two living documents.
